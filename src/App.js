...
const runGame = (grid, aliveCells) => {
  ...
  if(aliveCells.size){
    ...
    while(!current.done){
      ...
      if (col < COLS - 1) updateCell(row, col+1)
      if (row < ROWS - 1) updateCell(row+1, col)
      if (col < COLS - 1 && row > 0) updateCell(row-1, col+1)
      if (row < ROWS - 1 && col > 0) updateCell(row+1, col-1)
